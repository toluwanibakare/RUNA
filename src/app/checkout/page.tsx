...
export default function CheckoutPage() {
  const { packs, subtotal, deliveryFee, total } = useCart();
  ...
  return (
    <div className="space-y-5">
      ...
      <h1 className="text-xl font-bold text-[#0C231D]">Checkout - {packs.length} pack{packs.length !== 1 ? "s" : ""}</h1>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4 space-y-4">
        ...
        <div className="border-t border-[#E5E7EB] pt-3 space-y-1.5 text-sm">
          ...
          <div className="flex justify-between text-[#6B7280]"><span>Delivery ({packs.length} x {formatPrice(300)})</span><span className="text-[#0C231D]">{formatPrice(deliveryFee)}</span></div>
          ...
          <p className="text-xs text-[#6B7280]">Adding from another store creates a new pack and increases delivery fee.</p>
        </div>
      </div>
